...
from chara_utils import chord_backup_map, get_all_chords, hex_chords_to_list, list_chara_devices_with_desc, open_connection, chord_to_list
...
def prompt_user_to_select_device(devices: list[str], descriptions: list[str]) -> str | None:
    if len(devices) == 1:
        return devices[0]
    elif len(devices) == 0:
...
def update_chord_backup():
    ...
    structured_chords = hex_chords_to_list(chords)
